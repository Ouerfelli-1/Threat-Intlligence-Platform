...
interface TTP {
  ...
  // confidence still arrives from the backend on every TTP row but the UI
  // no longer renders it (operator dropped numeric confidence platform-wide).
  confidence: number;
  source: string;
}
...
type TabId = 'overview' | 'ttps' | 'iocs' | 'intel' | 'insight' | 'ransomware';
...
// All TTPs render with the same flat accent now that numeric confidence
// metrics are removed platform-wide. A row exists for a technique or it
// doesn't — there's no "low-confidence shadow" intermediate state.
const TTP_CELL_BG = 'rgba(88,166,255,0.10)';
const TTP_CELL_BORDER = 'rgba(88,166,255,0.25)';
...
/* ---- AI Insight tab ──────────────────────────────────────────────────────
 * Renders the same hunting-hypothesis / IOC / attack-flow output as the
 * threat detail page, generated by POST /actors/{id}/analyze.
 *
 * Cache-first by default — pressing "Generate insight" on a fresh actor
 * runs the three-leg pipeline ONCE and stores the row. Re-visiting the
 * tab serves the saved row without re-billing the AI. "Re-analyze"
 * forces a fresh run.
 */
function InsightTab({ actorId }: { actorId: string }) {
  const { data: insight, isLoading: insightLoading, mutate: mutateInsight } = useSWR<InsightEnvelope>(
    actorId ? `/actors/${actorId}/insight` : null,
    fetcher,
  ...
  const handleAnalyze = useCallback(async (force = false) => {
    setAnalyzing(true);
    try {
      await api.post(`/actors/${actorId}/analyze`, force ? { force: true } : {});
      mutateInsight();
    } finally { setAnalyzing(false); }
  }, [actorId, mutateInsight]);

  return (
    <div style={{ padding: 14 }}>
      {insightLoading && (
        <div style={{ textAlign: 'center', color: 'var(--text-4)', padding: 30 }}>Loading insight...</div>
      )}
      {!insightLoading && !insight && (
        <div className="card" style={{ padding: 24, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, color: 'var(--text-4)' }}>
          <Sparkles s={28} />
          <div style={{ fontSize: 13, color: 'var(--text-2)' }}>No AI insight yet</div>
            ...
            {TACTICS.map((tac) => (
              <div key={tac.id} style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                ...
                {(tacticMap[tac.id] ?? []).slice(0, 6).map((ttp) => (
                  <div key={ttp.technique_id} title={`${ttp.technique_id} — ${ttp.technique_name}`}
                       style={{ padding: '5px 6px', borderRadius: 4, border: `1px solid ${TTP_CELL_BORDER}`, background: TTP_CELL_BG, cursor: 'default', minHeight: 36 }}>
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        ...
        {tab === 'insight'    && <InsightTab actorId={actor.id} />}
